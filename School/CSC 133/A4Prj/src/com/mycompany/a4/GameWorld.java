...
public class GameWorld extends Observable implements IGameWorld
{
	...
	{
		BezierCurve bc = new BezierCurve(ColorUtil.MAGENTA, getHeight(), getWidth());
		...
		while(iter.hasNext())
		{
			...
			if(current instanceof PlayerShip)
			{
				...
				double y = ((PlayerShip)current).getLocation().getY();
				bc.setLocation(new Point2D(x, y));
				bc.setDirection(((PlayerShip) current).getDirection());
				bc.setSpeed(((PlayerShip)current).getSpeed() + 20);
				System.out.println("Fired curve");
				gameObj.add(bc);
			}
		}		
		notifyObservers();
	...
	public void tick()
	{
		...
		while(iter.hasNext())
		{
			...
			if(current instanceof Missile)
				if(((Missile) current).getFuel() <= 0)
					iter.remove();
			if(current instanceof BezierCurve)
				if(((BezierCurve) current).getFuel() <=0)
					iter.remove();
		}
		...
		if(!collisionVectorNPS.isEmpty())
		{
			...
			while(npsIterator.hasNext())
			{
				...
				else if (curObj instanceof BezierCurve)
					this.npsShotCurve(npsIterator, nps);
			}
		...
		if(!collisionVectorAsteroid.isEmpty())
		{
			...
			while(astIterator.hasNext())
			{
				...
				if(curObj instanceof Missile && !((Missile)curObj).isEnemy())
					...
					this.astShotCurve(astIterator, ast);
			}
	...
	public void fireNPS()
	{
		...
		while(iter.hasNext())
		{
			...
			if(current instanceof NonPlayerShip && ((NonPlayerShip)current).getMissileCount() > 0)
			{
					...
					double y = ((NonPlayerShip)current).getLocation().getY();
					mis.setLocation(new Point2D(x, y));
	...
	private void astShotCurve(Iterator<ICollider> astIterator, ICollider ast) {
		trash.add(ast);
		astIterator.remove();
		score = score + 100;
	...
	private void npsShotCurve(Iterator<ICollider> npsIterator, ICollider nps) {
		trash.add(nps);
		npsIterator.remove();
		totalNPS--;
		score = score + 500;
